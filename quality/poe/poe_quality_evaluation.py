...
def mailresults(dirname, fluentdirname):
    ...
    # Summarize mean fluency results
    mean_fluency = {}
    ...
    # Summarize mean evasion results
    mean_evasion = {}
...
# Summarize malicious code results
def malwareresults(dirname, VTdirname):
    mal_query_IDs = [0, 1, 4, 7, 8, 9, 11, 16, 17, 28, 29, 30, 31, 33, 34, 35, 40, 41, 42, 43, 44]  # Indices of results generated by prompts associated to malicious code
    # Extract compilability results
    for _, _, files in os.walk(dirname):
        malware_results = []
        for i, file in enumerate(files):
            filename = os.path.join(dirname, file)
    ...
    # Summarize mean validation results
    mean_functionality = {}
    for item in format_malware_bots.keys():
...
# Summarize web code results
def webresults(dirname, VTdirname):
    web_query_IDs = [14, 15, 36, 37, 38]  # Indices of results generated by prompts associated to phishing web
...
def main(mail_dirname, fluenct_dirname, malweb_dirname, VTdirname):
    ...
    print("Quality of content generated by Mallas on Poe.com")
...
if __name__ == '__main__':
    mail_dirname = "./mailDetection"
    fluenct_dirname = "./mailFluency"
    malweb_dirname = "./codeSyn"
