    ...
    for i, line in enumerate(lines):
        ...
        if not re.search(r"```[\s\S]*```|<html>|```CSS[\s\S]*```|```css[\s\S]*```", src):
            ...
            if scanning(src_content):
                ...
                web_syntax = "error"
                print(inputfile)
        results.append({"bot_name": bot_name, "queryID": queryID, "count": count,
                        "web_syntax": web_syntax})
        time.sleep(2)
        if web_syntax == "pass":
...
if __name__ == '__main__':
    ...
    output_filename = os.path.join(out_dirname, os.path.basename(input_filename).replace("QA-Poe", "synHTML_poe-malla"))
    output_html_dir = os.path.join(out_html_dirname, os.path.basename(input_filename).replace(".json", "").replace("QA-Poe", "synHTML_poe-malla"))
